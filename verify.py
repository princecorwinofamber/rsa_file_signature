...
filename = input("Please, enter the name of the file to verify: ")
public_key = tuple(map(int, input("Please, enter the public key: ").replace(" ", "").split(",")))
f = open(filename + ".signature", "r")
signature = int(f.read())
f.close()
f = open(filename, "rb")
if verify(f.read(), signature, public_key):
    print("The file is matching its signature.")
else:
    print("The file cannot be trusted.")
f.close()
